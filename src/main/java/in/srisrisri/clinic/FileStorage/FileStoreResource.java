 ...
 * @author ayush
 */
import in.srisrisri.clinic.jote.JoteEntity;
...
public class FileStoreResource {
    ...
    private static final String label = "fileStore";
    @Autowired
    ...
    public String submit(
            @RequestParam MultipartFile file, JoteEntity jotePassed) {
        System.out.println("id" + jotePassed.toString());

        try {
            file.transferTo(new File("/home/ayush/Desktop/" + file.getOriginalFilename()));
        } catch (Exception ex) {
            java.util.logging.Logger.getLogger(FileStoreResource.class.getName()).log(Level.SEVERE, null, ex);
        }
