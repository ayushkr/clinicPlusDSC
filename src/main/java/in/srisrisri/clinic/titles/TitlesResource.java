...
@RestController
@RequestMapping("/clinicPlus/api/titles")
public class TitlesResource {

    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    private final TitlesRepo repo;

    String label = "titles";

    public TitlesResource(TitlesRepo repo) {
        this.repo = repo;
    }
    ...
    @GetMapping("{id}")
    @ResponseBody
    public Optional<Titles> getById(@PathVariable("id") Long id) {
        logger.warn("getDoctorById ={} No= {}", new Object[]{label, id});
        Optional<Titles> item;
        if (id >= 0) {
            item = repo.findById(id);
        } else {

            Titles entityAfter = new Titles();
    ...
    public ResponseEntity<JsonResponse> PostMapping_one_json(
            @RequestBody Titles entityBefore
    ) {
...
//@ModelAttribute   @RequestPart
    ...
    public ResponseEntity<JsonResponse> PostMapping_one(
        ...
        try {
            ...
            Titles entityAfter = null;

            entityAfter = (Titles) repo.findById(entityBefore.getId()).get();
           

            BeanUtils.copyProperties(entityBefore, entityAfter);
            try {
                entityAfter = (Titles) repo.save(entityAfter);
    ...
    @GetMapping("")
    @ResponseBody
    public List<Titles> all() {
        logger.warn("REST getItems() , {} ", new Object[]{label});
        List<Titles> list = repo.findAll();
        return list;
    }

    @GetMapping("pageable")
    @ResponseBody
    public PageCover<Titles> pageable_general(
        ...
        Page<Titles> pageList = repo.findAll(pageable);
        PageCover<Titles> pageCover = new PageCover<>(pageList);
        pageCover.setSortColumn(sortColumn);
        pageCover.setSortOrder(sortOrder);
        pageCover.setModule(label);
//        Titles titles=new Titles();
//        titles.setModule("titles");
//        titles.setName(true);
